...
for w in range(12):
    ...
    for i in range(10):
        ...
        if w == (plot_batch - 1) and i == (plot_exp % 10):
            ...
            plt.stackplot(
                ...
                df['A'][exp_end+batch_start:exp_end+exp0+batch_start],
                df['B'][exp_end+batch_start:exp_end+exp0+batch_start],
                df['C'][exp_end+batch_start:exp_end+exp0+batch_start],
                df['D'][exp_end+batch_start:exp_end+exp0+batch_start],
                df['E'][exp_end+batch_start:exp_end+exp0+batch_start],
                labels=['0 - .20', '.21 - .40', '.40 - .60', '.60 - .80',
                        '.80 - 1'],
    ...
    if w == plot_batch:
        ...
        plt.title('Stack Area Plot Batch ' + str(plot_batch) +
                  ': Mean = ' + plot_mean + " and STD = " + plot_std)
        plt.xlim(0, 1000)
        plt.ylim(0, 100)
        plt.xlabel('Epoch')
        plt.ylabel('Agent Count')
        plt.savefig('Stack_b' + str(plot_batch) + '_e' + str(plot_exp) +
                   '.png')
        plt.show()
    exp_end += exp0
