...
- Outputs multi-line plots of Median SVO (with ± 1 Decile range) for each
    batch, containing all experimental runs in that batch.
"""
...
for w in range(12):
    ...
    for i in range(10):
        ...
        plt.plot(
            ...
            df['Median'][exp_end+batch_start:exp_end+exp0+batch_start],
            label=str(df[df.columns[2]][exp_end+batch_start]) + ",  " +
            str(round(df[df.columns[3]][exp_end+batch_start], 2)))
        plt.fill_between(
            df['Epoch'][exp_end+batch_start:exp_end+exp0+batch_start],
            df['Below'][exp_end+batch_start:exp_end+exp0+batch_start],
            df['Above'][exp_end+batch_start:exp_end+exp0+batch_start],
            alpha=.3)
        exp_end += exp0
    ...
    plt.title('SVO Median ± Decile of 10 Unimodal Distributions, Batch ' +
              str(w+1))
    plt.xlabel('Epoch')
    plt.ylabel('SVO')
    plt.xlim(0, 100)
