...
- Outputs multi-line plots of Agent Count per Epoch for each batch,
...
# 	to be changed to 54 for bimodal and  for trimodal.
for w in range(12):
    exp_end = 0
    for i in range(10):
        exp0 = 0
        ...
        plt.plot(
            ...
            agent_count[exp_end+batch_start:exp_end+exp0+batch_start],
            label="Mean = " + str(df[df.columns[2]][exp_end+batch_start]) +
                  " and STD = " +
    ...
    plt.xlim(0, 1000)
    plt.ylim(0, 100)
    plt.savefig('../plots/FinalCount' + str(w+1) + '.png')
    plt.show()
